...
By passing the \textbf{-v} flag the emulator will display the memory region starting at the pointer contained in special location (0x1000000 - the video pointer) at a fixed resolution (192x108) with full RGB colour (4 bytes per pixel, lower 3 are rgb). When a value is stored to the memory pointer, the display is updated.
