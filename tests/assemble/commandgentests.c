...
void ProcessBranchTests() {
    ...
    MapSet(symbols, "label", 8);
    MapSet(symbols, "wait", 4);
    
    InitFunctionGen();
    Vector text = NewEmptyVector();
    Vector data = NewEmptyVector();

    SingleProcess(symbols, 2, (Token[]){
...
void ProcessDataTransferTests() {
    ...
    assert((int)VectorGet(text, 2) == 0xE5B13004);
...
void ProcessAliasTests() {
    ...
    SingleProcess(symbols, 2, (Token[]){
        NewInstructionToken(COND_AL, INSTR_PSH),
        NewRegisterToken(8)
    }, text, data, 0, 4);
    SingleProcess(symbols, 1, (Token[]){
        NewInstructionToken(COND_AL, INSTR_HLT)
    }, text, data, 1, 4);
    SingleProcess(symbols, 2, (Token[]){
        NewInstructionToken(COND_AL, INSTR_POP),
        NewRegisterToken(5)
    }, text, data, 2, 4);
    SingleProcess(symbols, 1, (Token[]){
        NewInstructionToken(COND_LE, INSTR_RET),
    }, text, data, 3, 4);

    assert((int)VectorGet(text, 0) == 0xE52D8004);
    assert((int)VectorGet(text, 1) == 0x00000000);
    assert((int)VectorGet(text, 2) == 0xE49D5004);
    assert((int)VectorGet(text, 3) == 0xD1A0F00E);
